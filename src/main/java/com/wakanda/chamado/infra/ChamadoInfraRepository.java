...
public class ChamadoInfraRepository implements ChamadoRepository {
	...
	@Override
	public List<Chamado> buscaChamadosDoClientePorPrioridade(UUID idCliente, Prioridade prioridade) {
		log.info("[start] ChamadoInfraRepository - buscaChamadosDoClientePorPrioridade");
		Query query = new Query();
		query.addCriteria(Criteria.where("idCliente").is(idCliente));
		query.addCriteria(Criteria.where("prioridade").is(prioridade));
		List<Chamado> chamados = mongoTemplate.find(query, Chamado.class);
		log.info("[finish] ChamadoInfraRepository - buscaChamadosDoClientePorPrioridade");
		return chamados;
	}
	...
	@Override
	public List<Chamado> buscaChamadosDoTecnicoPorStatus(UUID idTecnico, StatusChamado status) {
		log.info("[start] ChamadoInfraRepository - buscaChamadosDoTecnicoPorStatus");
		Query query = new Query();
		query.addCriteria(Criteria.where("idTecnico").is(idTecnico));
		query.addCriteria(Criteria.where("status").is(status));
		List<Chamado> chamados = mongoTemplate.find(query, Chamado.class);
		log.info("[finish] ChamadoInfraRepository - buscaChamadosDoTecnicoPorStatus");
		return chamados;
	}
	
	@Override
	public List<Chamado> buscaChamadosDoClientePorStatus(UUID idCliente, StatusChamado status) {
		log.info("[start] ChamadoInfraRepository - buscaChamadosDoClientePorStatus");
		Query query = new Query();
		query.addCriteria(Criteria.where("idCliente").is(idCliente));
		query.addCriteria(Criteria.where("status").is(status));
		List<Chamado> chamados = mongoTemplate.find(query, Chamado.class);
		log.info("[finish] ChamadoInfraRepository - buscaChamadosDoClientePorStatus");
		return chamados;
	}
